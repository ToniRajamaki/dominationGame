...
EliteWorker::EliteWorker(const std::shared_ptr<Course::iGameEventHandler>& eventhandler,
...
std::string EliteWorker::getType() const
{
    return "EliteWorker";
}

bool EliteWorker::canBePlacedOnTile(const std::shared_ptr<Course::TileBase> &target) const
{
    return target->getOwner() == getOwner() and
            WorkerBase::canBePlacedOnTile(target);
}

void EliteWorker::doSpecialAction()
{

}

const Course::ResourceMapDouble EliteWorker::tileWorkAction()
{
