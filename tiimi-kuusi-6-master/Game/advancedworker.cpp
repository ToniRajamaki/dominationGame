...
AdvancedWorker::AdvancedWorker(const std::shared_ptr<Course::iGameEventHandler>& eventhandler,
...
std::string AdvancedWorker::getType() const
{
    return "AdvancedWorker";
}

bool AdvancedWorker::canBePlacedOnTile(const std::shared_ptr<Course::TileBase> &target) const
{
    return target->getOwner() == getOwner() and
            WorkerBase::canBePlacedOnTile(target);
}

void AdvancedWorker::doSpecialAction()
{

}

const Course::ResourceMapDouble AdvancedWorker::tileWorkAction()
{
    ...
    if ( events->modifyResource(player, Course::BasicResource::FOOD, -3) )
    {
        satisfaction = 0.5;
